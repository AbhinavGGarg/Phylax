// PHYLAX · containment console — talks DIRECTLY to InsForge edge functions
// (config.js → window.PHYLAX). The real PSI/MPC + model run server-side in the
// `demo-sweep` edge function; this drives the graph + timeline from its result.
...
const API = (window.PHYLAX && window.PHYLAX.base) || "";
const ANON = (window.PHYLAX && window.PHYLAX.anon) || "";
...
function buildGraph() {
  ...
  defs.innerHTML = `
    <linearGradient id="mem" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="rgba(53,224,212,0)"/><stop offset=".5" stop-color="rgba(53,224,212,.55)"/><stop offset="1" stop-color="rgba(53,224,212,0)"/></linearGradient>
    <radialGradient id="sealg" cx=".5" cy=".5" r=".5"><stop offset="0" stop-color="rgba(53,224,212,.28)"/><stop offset="1" stop-color="rgba(53,224,212,0)"/></radialGradient>`;
  ...
  for (const k of ["fintrust", "swiftcart", "pingline"]) { const a = P[k], b = P.seal, mx = (a.x + b.x) / 2; el("path", { id: "e-" + k, d: `M${a.x},${a.y} C ${mx},${a.y} ${mx},${b.y} ${b.x},${b.y}`, fill: "none", stroke: "var(--line)", "stroke-width": 1.4 }, edges); }
  ...
  for (const k of ["fintrust", "swiftcart", "pingline"]) { const a = P[k], c = P.cluster; el("path", { d: `M${c.x},${c.y} Q ${(a.x + c.x) / 2},${(a.y + c.y) / 2 - 40} ${a.x},${a.y}`, fill: "none", stroke: "var(--crimson)", "stroke-width": 1.3, "stroke-dasharray": "3 4", opacity: .8 }, arcs); }
  el("g", { id: "packets" }, g);
  ...
  function node(id, x, y, label, sub, r = 34) {
    ...
    el("text", { y: 4, "text-anchor": "middle", fill: "var(--ink)", "font-size": 13, "font-weight": 600 }, grp).textContent = label;
    if (sub) el("text", { y: r + 26, "text-anchor": "middle", fill: "var(--faint)", "font-size": 10.5, "font-family": "var(--mono)" }, grp).textContent = sub;
  }
  ...
  el("text", { y: 6, "text-anchor": "middle", fill: "var(--cyan)", "font-size": 34, "font-weight": 600, "font-family": "var(--mono)" }, seal).textContent = "0";
...
function setNodeState(id, s) {
  ...
  ring.setAttribute("stroke", map[s] || "var(--line)"); ring.setAttribute("stroke-width", s === "idle" ? 1 : 2);
...
function startPackets() {
  if (packetsOn) return; packetsOn = true;
  ["fintrust", "swiftcart", "pingline"].forEach((k, i) => spawnTimers.push(setInterval(() => { if (packetsOn) spawnPacket("e-" + k, "var(--cyan)", 1100, () => { pulseSeal(); spawnPacket("e-out", "var(--iris)", 800); }); }, 900 + i * 130)));
}
...
function phase(lab, sub, color = "var(--cyan)") { $("#phaselab").textContent = lab; $("#phasesub").textContent = sub || ""; $("#phaselab").style.color = color; $("#phasedot").style.background = color; $("#phasedot").style.boxShadow = `0 0 9px ${color}`; }
...
function renderSignals(parties) {
  const rows = ["fintrust", "swiftcart", "pingline"].map((slug) => {
    ...
    return `<div class="signal"><div class="top"><span class="org">${meta.name}</span><span class="tag mono">${meta.kind}</span></div>
      <div class="blurb">${p ? p.blurb : "Monitoring local signals…"}</div>
      <div class="solo"><span>solo risk</span><div class="meter"><i style="width:${solo != null ? Math.max(3, solo * 100) : 3}%;background:var(--emerald)"></i></div><span class="mono">${solo != null ? fmtPct(solo) : "—"}</span></div></div>`;
  }).join("");
  $("#signals").innerHTML = `<p class="muted" style="margin:2px 0 12px">Three platforms, three fragments. Each below its own action threshold.</p>` + rows;
}
...
function renderRisk(s) {
  $("#riskPct").textContent = fmtPct(s.risk_score); $("#riskBar").style.width = Math.max(4, s.risk_score * 100) + "%"; $("#confVal").textContent = fmtPct(s.confidence);
  $("#drivers").innerHTML = (s.contributions || []).slice(0, 5).map((c) => {
    const mag = Math.min(100, Math.abs(c.contribution) / 3.2 * 100), col = c.contribution >= 0 ? "var(--crimson)" : "var(--emerald)";
...
function renderActions() {
  ...
  $("#actions").innerHTML = state.actions.map((a) => {
    ...
    return `<div class="act"><div><div class="lbl">${a.action_type.replace(/_/g, " ")}</div><div class="sub">${nameOf[a.target_org] || a.target_org}</div></div>${tag}</div>`;
  }).join("");
  if (state.actions.length && state.actions.every((a) => a.status === "requested") && !$("#approveAllBtn")) {
    const btn = document.createElement("button"); btn.id = "approveAllBtn"; btn.className = "btn btn-primary"; btn.style.cssText = "width:100%;margin-top:4px"; btn.textContent = "Approve all (as target-org approvers)"; btn.onclick = approveAll; $("#actions").appendChild(btn);
  }
}
function renderReceipt() {
  const box = $("#receiptBox"); box.classList.remove("hidden");
  if (state.receipt) {
    box.innerHTML = `<div class="act" style="border-color:rgba(53,224,212,.4)"><div><div class="lbl">Incident receipt sealed</div><div class="sub">${short(state.receipt.receiptHash)}</div></div><span class="tag ${state.receiptVerified ? "cy" : "am"}">${state.receiptVerified ? "verified ✓" : "verifying…"}</span></div>`;
  } else {
    box.innerHTML = `<button class="btn" id="issueBtn" style="width:100%">Issue signed incident receipt</button>`; $("#issueBtn").onclick = issueReceipt;
  }
}
...
async function runSweep() {
  ...
  $("#finding").classList.add("hidden"); $("#findingEmpty").classList.remove("hidden");
  $("#actions").innerHTML = ""; $("#receiptBox").classList.add("hidden"); $("#drivers").innerHTML = ""; $("#riskPct").textContent = "—"; $("#riskBar").style.width = 0;
  ["fintrust", "swiftcart", "pingline", "phylax"].forEach((k) => setNodeState(k, "idle"));
  buildGraph();
  phase("Establishing protected session", "dispatching parties", "var(--cyan)"); tl("run.created");

  const sweep = fn("demo-sweep");          // real compute starts now
  await sleep(500);
  ["fintrust", "swiftcart", "pingline"].forEach((k, i) => setTimeout(() => { setNodeState(k, "ready"); tl("party.ready", ORG[k].name); }, i * 180));
  phase("Protected match · running", "commutative ECDH-PSI over opaque tokens", "var(--cyan)"); startPackets();

  let res;
  try { [res] = await Promise.all([sweep, sleep(3600)]); } catch (e) { phase("Run failed", e.message, "var(--crimson)"); resetBtn(); return; }
  if (!res || !res.runId) { phase("Run failed", (res && res.error && res.error.message) || "sweep error", "var(--crimson)"); resetBtn(); return; }
  ...
  $("#runIdTag").textContent = res.runId.slice(0, 8); $("#runIdTag").className = "tag cy";
  ...
  tl("risk.scored", `${fmtPct(s.risk_score)} risk`);
  state.actions.forEach((a) => tl("action.requested", a.action_type.replace(/_/g, " ")));
  renderActions();
  phase("Risk scored · awaiting approval", "human decision required", "var(--amber)");
...
async function approveAll() {
  ...
  state.actions = state.actions.map((a) => ({ ...a, status: "approved" }));
  state.actions.forEach((a) => tl("action.approved", a.action_type.replace(/_/g, " ")));
  renderActions();
  if (res.status === "actioned") { tl("run.actioned"); phase("Actions executed", "issue receipt to seal", "var(--emerald)"); $("#sweepstate").textContent = "actioned"; renderReceipt(); }
}
async function issueReceipt() {
  const b = $("#issueBtn"); if (b) { b.disabled = true; b.textContent = "Sealing…"; }
  const r = await fn("demo-receipt", { runId: state.runId });
  state.receipt = r; renderReceipt();
  const v = await fn("verify-receipt", { runId: state.runId, receipt: r.receipt });
  state.receiptVerified = v.valid === true; renderReceipt();
  tl("receipt.issued", short(r.receiptHash)); phase("Sealed", "immutable receipt issued", "var(--cyan)"); $("#sweepstate").textContent = "sealed";
}

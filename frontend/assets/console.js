...
function buildGraph() {
  ...
  defs.innerHTML = `
    <linearGradient id="mem" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0" stop-color="rgba(30,45,255,0)"/><stop offset=".5" stop-color="rgba(30,45,255,.55)"/>
      <stop offset="1" stop-color="rgba(30,45,255,0)"/></linearGradient>
    <radialGradient id="sealg" cx=".5" cy=".5" r=".5">
      <stop offset="0" stop-color="rgba(30,45,255,.28)"/><stop offset="1" stop-color="rgba(30,45,255,0)"/></radialGradient>
  ...
  for (const k of ["fintrust", "swiftcart", "pingline"]) {
    ...
    const mx = (a.x + b.x) / 2;
    el("path", { id: "e-" + k, d: `M${a.x},${a.y} C ${mx},${a.y} ${mx},${b.y} ${b.x},${b.y}`, fill: "none", stroke: "rgba(0,0,0,0.08)", "stroke-width": 1.4, class: "edge" }, edges);
  }
  ...
  for (const k of ["fintrust", "swiftcart", "pingline"]) {
    ...
    el("path", { d: `M${c.x},${c.y} Q ${(a.x + c.x) / 2},${(a.y + c.y) / 2 - 40} ${a.x},${a.y}`, fill: "none", stroke: "#DC2626", "stroke-width": 1.3, "stroke-dasharray": "3 4", opacity: .8 }, arcs);
  }
  ...
  function node(id, x, y, label, sub, r = 34) {
    ...
    const t = el("text", { y: 4, "text-anchor": "middle", fill: "#0A0A1A", "font-size": 13, "font-weight": 600 }, grp);
    t.textContent = label;
    if (sub) { const s = el("text", { y: r + 26, "text-anchor": "middle", fill: "#999AA8", "font-size": 10.5, "font-family": "monospace" }, grp); s.textContent = sub; }
    return grp;
  }
  ...
  el("circle", { r: 44, fill: "rgba(255,255,255,0.95)", stroke: "#1E2DFF", "stroke-width": 1.5, id: "sealring" }, seal);
  const z = el("text", { y: 6, "text-anchor": "middle", fill: "#1E2DFF", "font-size": 34, "font-weight": 600, "font-family": "monospace" }, seal);
...
function setNodeState(id, s) {
  ...
  ring.setAttribute("stroke", map[s] || "rgba(0,0,0,0.08)");
...
function startPackets() {
  ...
  orgs.forEach((k, i) => {
    const timer = setInterval(() => {
      ...
      spawnPacket("e-" + k, "#1E2DFF", 1100, () => {
        pulseSeal();
        spawnPacket("e-out", "#1E2DFF", 800); // raw → sealed → opaque
      });
    }, 900 + i * 130);
...
function revealCampaign() {
  ...
  $("#sealring").setAttribute("stroke", "#1E2DFF"); // 0 holds, still cyan/sealed
}
...
function phase(lab, sub, color = "#1E2DFF") {
...
function renderSignals(parties) {
  ...
  const rows = ["fintrust", "swiftcart", "pingline"].map((slug) => {
    ...
    return `<div class="signal">
      ...
      <div class="solo"><span>solo risk</span><div class="meter"><i style="width:${solo != null ? Math.max(3, solo * 100) : 3}%;background:#169B5A"></i></div><span class="mono">${solo != null ? fmtPct(solo) : "—"}</span></div>
    </div>`;
  }).join("");
...
function renderRisk(s) {
  ...
  const pct = sum.risk_score * 100;
  if (window._animAnimateCount) {
  ...
  const drivers = (sum.contributions || []).slice(0, 5).map((c) => {
    ...
    const col = c.contribution >= 0 ? "#DC2626" : "#169B5A";
...
async function renderReceiptBox(s) {
  ...
  if (s.run?.status === "receipted" && state.receipt) {
    ...
    box.innerHTML = `<div class="act" style="border-color:rgba(30,45,255,.4)"><div><div class="lbl">Incident receipt sealed</div><div class="sub">${short(state.receipt.receiptHash)}</div></div>
...
function handleEvent(event, data) {
  ...
  switch (event) {
    case "protected_match.started": phase("Protected match · running", "commutative ECDH-PSI over opaque tokens", "#1E2DFF"); startPackets(); break;
    case "party.ready": if (data.orgId) { const o = (state.config.orgs || []).find((x) => x.id === data.orgId); if (o) setNodeState(o.slug, "ready"); } break;
    case "protected_match.completed": stopPackets(); revealCampaign(); phase("Campaign proven", `cardinality ${data.matchCardinality ?? ""}`, "#DC2626"); setNodeState("phylax", "active"); refresh(); break;
    case "risk.scored": phase("Risk scored · awaiting approval", "human decision required", "#CC6C00"); refresh(); break;
    case "run.awaiting_approval": refresh(); break;
    case "action.approved": case "action.rejected": refresh(); break;
    case "run.actioned": phase("Actions executed", "issue receipt to seal", "#169B5A"); refresh(); break;
    case "receipt.issued": phase("Sealed", "immutable receipt issued", "#1E2DFF"); $("#sweepstate").textContent = "sealed"; break;
    case "run.failed": phase("Run failed", data.reason || "", "#DC2626"); break;
  }
}
...
async function runSweep() {
  ...
  phase("Establishing protected session", "dispatching parties", "#1E2DFF");
  ...
  try {
    ...
    phase("Run failed", e.message, "#DC2626"); $("#runBtn").disabled = false; $("#runBtn").textContent = "Run Protected Sweep";
  }
}
